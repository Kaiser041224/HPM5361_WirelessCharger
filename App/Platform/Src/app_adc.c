...
int app_adc_set_pmt_trigger_position(app_adc_inst_t inst, float position_ratio) {
    ...
    if (inst == APP_ADC_INST_0) {
        return intf_hrpwm_set_trigger_cmp_position(
            HRPWM_INST_LCC, APP_ADC_PMT_TRIGGER_CMP_INDEX_PWM0, position_ratio);
    }

    return intf_hrpwm_set_trigger_cmp_position(
        HRPWM_INST_BUCKBOOST, APP_ADC_PMT_TRIGGER_CMP_INDEX_PWM1, position_ratio);
}
...
void app_adc_init(void) {
    ...
    (void)intf_hrpwm_config_trigger_cmp(
        HRPWM_INST_LCC, APP_ADC_PMT_TRIGGER_CMP_INDEX_PWM0, APP_ADC_PMT_POSITION_RATIO_ADC0);
    (void)intf_hrpwm_config_trigger_cmp(
        HRPWM_INST_BUCKBOOST, APP_ADC_PMT_TRIGGER_CMP_INDEX_PWM1, APP_ADC_PMT_POSITION_RATIO_ADC1);
}
