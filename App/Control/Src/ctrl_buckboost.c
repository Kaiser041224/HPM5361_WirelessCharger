...
static void buckboost_modulate(
    float v_cmd, float v_in, float d_min, float d_max, float* out_da, float* out_db) {
    float inv_sum = 1.0f / (v_in + v_cmd);
    float da = (d_max * v_cmd) * inv_sum;
    float db = (d_max * v_in) * inv_sum;
...
ATTR_RAMFUNC
void ctrl_buckboost_modulate(ctrl_buckboost_t* ctrl, float v_cmd, float v_in, float vlink) {
    if (ctrl == NULL || !ctrl->state.enabled) {
        return;
    }
    if (v_in <= 0.0f || v_cmd <= BUCKBOOST_V_CMD_MIN) {
        return;
    }
...
void ctrl_buckboost_update_current(ctrl_buckboost_t* ctrl, float i_l, float v_in, float vlink) {
    ...
    if (v_cmd > ctrl->params.v_out_limit_v) {
        v_cmd = ctrl->params.v_out_limit_v;
    }
...
void ctrl_buckboost_set_il_target(ctrl_buckboost_t* ctrl, float target_a) {
    if (ctrl == NULL)
        return;
    float new_ref = clampf(target_a, -ctrl->params.i_l_limit_a, ctrl->params.i_l_limit_a);
